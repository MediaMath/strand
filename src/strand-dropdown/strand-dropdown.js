...
(function(scope) {
  ...
  scope.Dropdown = Polymer({
    ...
    observers: ["_checkSizing(data.length)"],
    ...
    open: function(silent) {
      var inherited = BehaviorUtils.findSuper(
        StrandTraits.PositionableDropdown,
        "open"
      );
    ...
    _selectItemByValue: function() {
      Polymer.RenderStatus.afterNextRender(this, function() {
        var item = null;
        // rely on the current this.value for quick update scenarios
        // such as item refresh in an item-recycler
    ...
    _selectedIndexChanged: function(newIndex, oldIndex) {
      if (typeof newIndex === "number") {
        ...
        var value = newSelected.value
          ? newSelected.value.toString()
          : newSelected.textContent.trim();
        var name = newSelected.name
          ? newSelected.name
          : newSelected.textContent.trim();
    ...
    _updateTitle: function(selectedIndex) {
      if (typeof selectedIndex === "number") {
        ...
        if (selectedItem) {
          var availableArea =
              this.buttonWidth + this.borderWidth - this.paddingWidth,
