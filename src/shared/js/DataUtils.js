...
(function () {
	...
	DataUtils  = {
		...
		compoundObserverFactory: function(target, callback, _observer) {
			var mo = _observer || new MultiObserver();
			if (_isType(target, "object")) {
				mo.addObserver(new ObjectObserver(target));
			}
			if (_isType(target, "array")) {
				mo.addObserver(new ArrayObserver(target));
			}
			for(var i in target) {
				var item = target[i];
				if (_isType(item, "object") || _isType(item, "array")) {
					compoundObserverFactory(item, callback, mo);
				}
			}
			return mo;
		},
